...
// Uses your OWN button design but triggers Google OAuth under the hood
...
// Google SVG logo — no external dependency
const GoogleIcon = () => (
...
export default function GoogleAuthButton({ label = "Continue with Google", className = "", redirectTo = "/dashboard" }) {
  ...
  return (
    <button
      ...
      onClick={handleClick}
      disabled={loading}
      className={`flex w-full items-center justify-center gap-3 rounded-xl px-4 py-3 text-sm font-semibold transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed ${className}`}
      style={{
    ...
    >
      {loading ? (
        <svg className="h-5 w-5 animate-spin" viewBox="0 0 24 24" fill="none" style={{ color: 'var(--text-1)' }}>
